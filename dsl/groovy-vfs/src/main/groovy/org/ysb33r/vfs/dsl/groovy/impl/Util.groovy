...
class Util {
    ...
    static final Pattern OPTION_REGEX = FileSystemOptions.OPTION_REGEX

	/** Resolves a filesystem entity from a URI.
	 ...
	 */
	static VfsURI resolveURI (final Map<String,?> properties=[:], final FileSystemOptions defaultFSOptions, final Object uri) {

        FileSystemOptions opts
        if(uri instanceof VfsURI && properties) {
            opts = ((VfsURI)uri).fileSystemOptions
            opts.addAll(defaultFSOptions)
            opts.addAll(properties)
        } else if (uri instanceof VfsURI) {
            return (VfsURI)uri
        } else if(properties) {
...
//    }
     ...
     * @since 1.0
     */
    static VfsURI addRelativePath(VfsURI fo,CharSequence cs) {
        throw new URIException(fo.toString(),"addRelativePath() needs implementation")
...
//    }
     ...
     * @since 1.0
     */
    static boolean localURI(VfsURI uri) {
        FileSystems.default == uri.path.fileSystem
