...
import org.ysb33r.nio.provider.core.FileSystemProviderWithRegistry
import org.ysb33r.nio.provider.core.FileSystemRegistry
import org.ysb33r.nio.provider.core.SimpleFileSystemRegistry
...
import org.ysb33r.nio.provider.ram.internal.NamedEntry
...
/** A RAM Filesystem consists of a simple URI such as {@code ram://Identifier/path/file}.
 *
 * The scheme can be overridden at rn time by setting the system property
 * {@code org.ysb33r.bio.provider,ram.scheme} to something other than {@code ram}.
 *
 */
@CompileStatic
class RamFileSystemProvider extends AbstractFileSystemProviderWithRegistry /*implements FileSystemProviderWithRegistry*/ {

    RamFileSystemProvider() {
        super('ram',new SimpleFileSystemRegistry())
    }
     ...
     */
    @Override
    protected FileSystem doCreateFileSystem(final URI uri, Map<String, ?> env)  {
        new RamFileSystem(env,getKey(uri),this)
    }
    ...
    Path getPath(URI uri) {
        ...
        def elements = extractFilePath(uri).split(fs.separator)
        if(elements.size() == 1) {
            fs.getPath(elements[0])
        } else {
            fs.getPath(elements[0],elements[1..-1] as String[])
        }
    }
     ...
     * java.nio.file.StandardOpenOption # CREATE_NEW CREATE_NEW} option is specified
    ...
    SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs) throws IOException {
        if(!path.nameCount) {
            throw new IllegalArgumentException("Cannot open a channel for an empty path")
        }

        boolean create = options.find { it == StandardOpenOption.CREATE }
        boolean createNew = options.find { it == StandardOpenOption.CREATE_NEW }
        boolean deleteOnClose = options.find { it == StandardOpenOption.DELETE_ON_CLOSE }

        NamedEntry entry
        try {
            entry = getRamFileNamedEntry((RamFilePath)path)
            if(createNew) {
                throw new FileAlreadyExistsException("Cannot create file ${path} as it already exists")
            }
        } catch(final IOException e) {
            if(create) {
                entry = getRamFileNamedEntry((RamFilePath)path.parent)
            } else {
                throw new IOException("Cannot open a channel for non-existing file ${path}",e)
            }
        }
        // StandardOpenOption.CREATE_NEW

        // Are all of the options valid?

        // || it == StandardOpenOption.CREATE_NEW }

        // Ask filesystem whether Path exists and if it does return a object which can be opered upon

//        new RamFileSeekableByteChannel((File)entry,deleteOnClose)
        null
    }
     ...
     * specified by the {@link Files#copy(Path, Path, java.nio.file.CopyOption [ ])} method
    ...
    /**
     * Always {@code false} as the RAM filesystem does not have notion of hidden files.
     *
    ...
    FileStore getFileStore(Path path) throws IOException {
        ((RamFileSystem)(path.fileSystem)).getFileStoreByPath((RamFilePath)path)
    }
    ...
    // Transforms a path to an entry in the file store
    // Throws IOException is parent segments are missing.
    private Entry getRamFileEntry(final RamFilePath path) {
        RamFileStore fs = (RamFileStore)getFileStore(path)
        fs.ramFileData.locateEntryFromPath((RamFilePath)(path.toAbsolutePath()))
    }

    private NamedEntry getRamFileNamedEntry(final RamFilePath path) {
        RamFileStore fs = (RamFileStore)getFileStore(path)
        fs.ramFileData.locateContainerByPath((RamFilePath)(path.toAbsolutePath()),0)
    }

    private boolean ramFileExists(final RamFilePath path) {
        RamFileStore fs = (RamFileStore)getFileStore(path)
        fs.ramFileData.exists(path)
    }
}
